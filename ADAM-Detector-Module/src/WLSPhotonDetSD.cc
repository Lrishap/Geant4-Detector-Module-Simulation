...
// for outputs C++ [later replace by ROOT NtUPLES]

#include <iostream>
#include <fstream> // Required for file handling
#include "G4ios.hh" // Required for G4cout
// In your other code where you want to use this:
#include <sstream>
...
G4bool WLSPhotonDetSD::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
 ...
 // Get the event number
  ...
  CopyNo = theTouchable->GetCopyNumber(); //added to introduce ID

  // Creating the hit and add it to the collection

  // fPhotonDetHitCollection->insert( new WLSPhotonDetHit(photonExit, photonArrive, arrivalTime, energy,CopyNo)); //removed cf. memory constraint 

  fPhotonDetHitCollection->insert( new WLSPhotonDetHit(energy,CopyNo)); //removed cf. memory constraint 


  auto analysisManager = G4AnalysisManager::Instance();
  analysisManager->FillNtupleDColumn(1,2,eventNumber); //eventID
