...
void WLSEventAction::EndOfEventAction(const G4Event* evt)
{
  ...
  analysisManager->FillNtupleDColumn(1,0,mppcHC->entries()); //hits
 ...
 // Get the event number
    G4int eveNo = G4RunManager::GetRunManager()->GetCurrentEvent()->GetEventID();

   analysisManager->FillNtupleDColumn(0,0,pdHit->GetEnergy());
   analysisManager->FillNtupleDColumn(0,1,pdHit->GetDetectorID());
   analysisManager->FillNtupleDColumn(0,2,eveNo);
