...
/* Update 3/05/2024  ---> ommision of time component due to memory contraints and irrelevance
If needed you may compare to MAPP-2 in backup folder
*/
...
WLSPhotonDetHit::WLSPhotonDetHit()
{
 ...
 // fPosExit;  //= G4ThreeVector(0., 0., 0.);
  fDetectorID; // = 0;  declare check [seems to be working okay]

}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

// WLSPhotonDetHit::WLSPhotonDetHit(G4ThreeVector pExit, G4ThreeVector pArrive, G4double pTime, G4double pEnergy ,G4int DetectorID)

WLSPhotonDetHit::WLSPhotonDetHit(G4double pEnergy ,G4int DetectorID)
...
G4bool WLSPhotonDetHit::operator==(const WLSPhotonDetHit& right) const
{
  ...
  //        fArrivalTime == right.fArrivalTime && fEnergy == right.fEnergy && fDetectorID == right.fDetectorID;
  return fEnergy == right.fEnergy && fDetectorID == right.fDetectorID;         
}
