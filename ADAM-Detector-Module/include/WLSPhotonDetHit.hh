...
class WLSPhotonDetHit : public G4VHit
{
 public:
  WLSPhotonDetHit();
  //WLSPhotonDetHit(G4ThreeVector pExit, G4ThreeVector pArrive, G4double pTime, G4double pEnergy , G4int DetectorID);
  WLSPhotonDetHit(G4double pEnergy , G4int DetectorID);

  // due to memory constraints(possible) and time constraint I am removing all time intervels from detector hits 
  ...
  //added to access DetectorID
  inline void SetDetectorID(G4int detid) {fDetectorID = detid;}
  inline G4double GetDetectorID() { return fDetectorID; }

// added to access EventID
  // inline void SetEventID(G4int eveid) {fEventID = eveid;}
  // inline G4double GetEventID() { return fEventID; }
